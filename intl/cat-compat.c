/* Код совместимости для интерфейса gettext-using-catgets.
...
/* Название используемого в настоящее время каталога.  */
static const char *catalog_name = default_catalog_name;

/* Получить идентификатор для данной строки. Если не найден, верните -1.  */
static int msg_to_cat_id PARAMS ((const char *msg));

/* Замена систем без этой функции в их библиотеке C.  */
#if !_LIBC && !HAVE_STPCPY
static char *stpcpy PARAMS ((char *dest, const char *src));
#endif


/* Установить текущий используемый домен / каталог.  */
char *
     ...
     const char *domainname;
{
  ...
  /* Посмотрите, спрашивается ли имя текущего используемого домена.  */
  if (domainname == NULL)
  ...
  /* Вычислить длину добавленного элемента пути.  */
  ...
  if (new_catalog == (nl_catd) -1)
    {
      /* Поиск NLSPATH не работал, попробуйте абсолютный путь */
     ...
     const char *dirname;
{
  ...
  /* Здесь нет особого смысла, но для совместимости сделайте это.  */
  if (domainname == NULL)
    return NULL;

  /* Вычислить длину добавленного элемента пути. Если мы используем setenv, нам не нужно
     первые байты для NLSPATH =, но зачем усложнять код для этого
     арахис.  */
...
# else
  putenv (new_val);
  /* * Не * освобождайте только что введенную запись среды. Это использовано
     впредь.   */
# endif
     ...
     const char *msg;
{
  ...
  /* Получите сообщение из каталога. Мы всегда используем набор №1.
     Идентификатор сообщения вычисляется функцией msg_to_cat_id
     который работает с таблицей, созданной `po-to-tbl '.  */
...
/* Просмотрите таблицу _msg_tbl, в которой есть записи _msg_tbl_length
   для одного, равного msg. Если он найден, верните ID. В случае, когда
   строка не найдена, верните -1.  */
static int
msg_to_cat_id (msg)
     const char *msg;
{
