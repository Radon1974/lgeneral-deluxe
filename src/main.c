...
/* FIXME уже использовал дважды! время для консолидации */
#ifdef __GNUC__
...
static void show_title()
{
    ...
    SDL_Surface *back = 0;
    char path[MAX_PATH];
    search_file_name( path, 0,"title", "", "Theme", 'i' );
    if ( ( back = load_surf( path, SDL_SWSURFACE, 0, 0, 0, 0 ) ) ) {
        FULL_DEST( sdl.screen );
        FULL_SOURCE( back );
        blit_surf();
    }
    search_file_name( path, 0, "font_credit", "", "Theme", 'i' );
...
/* прерывается с заданным сообщением об ошибке и кодом выхода 1 */
...
int main(int argc, char *argv[])
{
    ...
    /* установить случайный источник */
    set_random_seed();

    /* проверьте путь к каталогу конфигурации и загрузите конфигурацию */
    ...
    /* показать название lgd */
    if (!suppress_title) show_title();

    /* переключитесь на разрешение конфигураций */
    sdl.num_vmodes = get_video_modes( &sdl.vmodes ); /* всегда успешно */
    set_video_mode( config.width, config.height, config.fullscreen );

#ifdef WITH_SOUND
    /* инициировать аудиоустройство */
    audio_open();
