...
int cur_weather;        /* погода текущего поворота */
int turn;               /* current turn id */
List *units = 0;        /* активные юниты */
List *vis_units = 0;    /* все юниты, замеченные текущим игроком */
List *avail_units = 0;  /* доступные юниты для развертывания текущим игроком */
List *reinf = 0;        /* подкрепления - единицы с задержкой (если задержка равна 0
                           юнит становится доступной в avail_units, если игрок
                           контролирует его. */
Player *player = 0;     /* текущий игрок */
int log_x, log_y;       /* позиция, где рисовать информацию журнала */
char *scen_domain;	/* домен этот сценарий был загружен под */
/* УСЛОВИЯ ПОБЕДЫ */
char scen_result[64] = "";  /* результат сценария сохраняется здесь */
char scen_message[128] = ""; /* последнее сообщение перенесены сюда */
int vcond_check_type = 0;   /* испытайте условия победы этого хода */
VCond *vconds = 0;          /* условия победы */
int vcond_count = 0;
int cheat_endscn = 0;       /* переменная, используемая для определения конечного сценария по чит-коду */
int *casualties;	/* сумма потерь сгруппированных по классу подразделения и игроку */
...
/*
====================================================================
Проверьте, заполнено ли субподрядное условие.
====================================================================
*/
static int subcond_check( VSubCond *cond )
{
    ...
    switch ( cond->type ) {
        ...
        case VSUBCOND_UNITS_KILLED:
            /* до тех пор, пока есть единицы, использующие этот тег, это условие
        ...
        case VSUBCOND_UNITS_SAVED:
            /* мы подсчитали количество единиц с этим тегом так что посчитайте еще раз
               а если одного не хватает: бинго */
...
/** Установите no_purchase для всех стран, которые либо не имеют возможности покупки
 * юнита в текущей библиотеке единиц измерения или не имеют ни флага, ни единицы измерения под
 * контроль (при этом отсутствуют в данном сценарии). */
void update_nations_purchase_flag()
{
	...
	/* очистить флаг-неправильно использовать его для проверок ниже */
	for (i = 0; i < nation_count; i++)
		nations[i].no_purchase = 0;

	/* отметьте все страны, которым назначена хотя бы одна запись */
	list_reset( unit_lib );
	while ((e = list_next( unit_lib )))
		if (e->nation != -1)
			nations[e->nation].no_purchase |= 0x01;

	/* отметьте все страны, у которых есть хотя бы один юнит/флаг под контролем */
	list_reset( units );
	...
	/* результаты карты - покупка в порядке, если установлены оба флага */
	for (i = 0; i < nation_count; i++)
...
/*
====================================================================
Загрузите сценарий в старом формате lgscn.
====================================================================
*/
int scen_load_lgscn( const char *fname, const char *path )
{
    ...
    Unit *unit;
    Unit unit_base; /* используется для хранения значений для юнита */
    int unit_delayed = 0;
    ...
    if ( !parser_get_pdata( pd, "unit_db", &sub ) ) {
        /* проверьте сам файл сценария, но только для основной записи */
    ...
    if ( !parser_get_value( pd, "map", &str, 0 ) ) {
        sprintf(str, "../scenarios/%s", fname); /* проверьте сам файл сценария */
    }
    ...
    while ( ( sub = list_next( entries ) ) ) {
        ...
        if ( STRCMP( str, "right" ) ) /* все они еще не реализованы */
	...
	/* Загрузите информацию о престиже, если таковая имеется. Если такового нет, покажите предупреждение. */
	...
	if ( parser_get_values( sub, "prestige", &values ) ) {
		...
		player->cur_prestige = 0; /* будет скорректировано в начале хода */
    ...
    /* сброс vic conds не может быть выполнен в scen_delete (), как это называется
       перед проверкой */
    scen_result[0] = 0;
    scen_message[0] = 0;
    /* условия подсчета */
    ...
    list_reset( entries ); i = 1; /* самое первое условие-это условие else */
    while ( ( pd_vcond = list_next( entries ) ) ) {
            ...
            if ( parser_get_pdata( pd_vcond, "and", &pd_bind ) ) {
                ...
                while ( ( pd_vsubcond = list_next( pd_bind->entries ) ) ) {
                    ...
                    if ( STRCMP( pd_vsubcond->name, "units_saved" ) ) {
                        ...
                        if ( !parser_get_value( pd_vsubcond, "tag", &str, 0 ) ) goto parser_failure;
                        strcpy_lt( vconds[i].subconds_and[j].tag, str, 31 );
                        vconds[i].subconds_and[j].count = 0; /* юнит будет вычислен */
                    }
                    j++;
                }
            }
            ...
            if ( parser_get_pdata( pd_vcond, "or", &pd_bind ) ) {
                ...
                while ( ( pd_vsubcond = list_next( pd_bind->entries ) ) ) {
                    ...
                    if ( STRCMP( pd_vsubcond->name, "units_saved" ) ) {
                        ...
                        if ( !parser_get_value( pd_vsubcond, "tag", &str, 0 ) ) goto parser_failure;
                        strcpy_lt( vconds[i].subconds_or[j].tag, str, 31 );
                        vconds[i].subconds_or[j].count = 0; /* юниты будут подсчитаны */
                    }
                    j++;
                }
            }
        ...
        while ( ( entry = list_next( reinf ) ) )
            if ( (camp_loaded == CONTINUE_CAMPAIGN) || ((camp_loaded == RESTART_CAMPAIGN) && entry->core == STARTING_CORE) )
            {
                ...
                if (camp_loaded != NO_CAMPAIGN)
                {
                    ...
                    unit->killed = 2;
                /* поместить объект в список доступных объектов */
                    list_add( units, unit );
                }
                entry->core = CORE;
            }
    ...
    while ( ( sub = list_next( entries ) ) ) {
        ...
        if ( parser_get_value( sub, "tag", &str, 0 ) ) {
            ...
            /* проверьте все субподряды на UNITS_SAVED и увеличьте счетчик
               если это подразделение союзное */
               ...
               ( (camp_loaded != NO_CAMPAIGN) && STRCMP(camp_cur_scen->id, camp_first) && config.use_core_units ) )
        {
            ...
            /* поместите подразделение в список активных или подкреплений или список доступных подразделений */
            if ( !unit_delayed ) {
                list_add( units, unit );
    ...
    /* проверьте, какие страны могут делать покупки для этого сценария */
...
/*
====================================================================
Загрузите сценарий в новом формате lgdscn.
====================================================================
*/
int scen_load_lgdscn( const char *fname, const char *path )
{
    ...
    while (load_line(inf,line,utf16)>=0)
    {
        ...
        //Block#1 общие сценарные данные
        ...
        //Block#4 распределение престижа
                ...
                if ( strcmp( tokens[2], "and" ) == 0 )
                {
                    ...
                    for ( j = 0; j < vconds[cur_vic_cond].sub_and_count; j++ ) {
                        ...
                        if ( strcmp( tokens[3 + j * 6], "units_saved" ) == 0 ) {
                            ...
                            vconds[cur_vic_cond].subconds_and[j].player = player_get_by_id( tokens[3 + 1 + j * 6] );
                            strcpy_lt( vconds[cur_vic_cond].subconds_and[j].tag, tokens[3 + 5 + j * 6], 31 );
                            vconds[cur_vic_cond].subconds_and[j].count = 0; /* юниты будут подсчитаны */
                        }
                    }
                }
                /* или связь */
                if ( strcmp( tokens[2], "or" ) == 0 )
                {
                    ...
                    for ( j = 0; j < vconds[cur_vic_cond].sub_or_count; j++ ) {
                        ...
                        if ( strcmp( tokens[3 + j * 6], "units_saved" ) == 0 ) {
                            ...
                            vconds[cur_vic_cond].subconds_or[j].player = player_get_by_id( tokens[3 + 1 + j * 6] );
                            strcpy_lt( vconds[cur_vic_cond].subconds_or[j].tag, tokens[3 + 5 + j * 6], 31 );
                            vconds[cur_vic_cond].subconds_or[j].count = 0; /* юниты будут подсчитаны */
                        }
                    }
                }
                cur_vic_cond++;
...
/*
====================================================================
Загрузите описание сценария в старом формате lgscn.
====================================================================
*/
...
/*
====================================================================
Загрузите описание сценария в новом формате lgdscn.
====================================================================
*/
...
/*
====================================================================
Очистить указатели в 'setup' которые были загружены scen_load_info()
====================================================================
*/
...
/*
====================================================================
Установите текущую атаку и движение. Если SCEN_PREP_UNIT_FIRST -
прошел окоп без доработок. Юниты не поставляются
или убили здесь!
====================================================================
*/
void scen_prep_unit( Unit *unit, int type )
{
    ...
    /* юнит не может быть развернут */
    unit->fresh_deploy = 0;
    /* удалить подавление хода */
    unit->turn_suppr = 0;
    /* разрешить действия */
    unit_unmount( unit );
    /* отметьте кнопку Instant_purchase и остановите действие недавно размещенных юнитов */
    if (type == SCEN_PREP_UNIT_DEPLOY && config.purchase == INSTANT_PURCHASE)   //юниты были доступны к растановке сразу после покупки
    {
        unit->cur_mov = 0;
    ...
    else
    {
        ...
        /* если у нас плохая погода, соотношение mov: fuel будет 1: 2
         * для наземных подразделений
         */
    ...
    /* если юнит подготовлен нормально, проверьте окопку */
...
/*
====================================================================
Проверьте, полностью ли выполнены условия победы, и если да, то
верните True. Затем "результат" используется
для определения следующего сценария кампании.
Если задано значение 'after_last_turn', то это проверка, вызываемая end_turn().
Если ни одно условие не заполнено, используется условие else (самое
первое).
====================================================================
*/
...
/*
====================================================================
Сообщение о возврате результата
====================================================================
*/
...
/*
====================================================================
Проверьте уровень питания юнита. (шестигранные плитки с SUPPLY_GROUND
имеют 100% запас)
====================================================================
*/
...
/*
====================================================================
Добавьте потери для юнитов. Учитывайте юниты и транспортные классы.
====================================================================
*/
...
/*
====================================================================
Panzer General генератор случайных погода, используя алгоритм обнаружен и
документально @Rudankort на
...
{
    /* счетчик ходов налево в текущий погодный период */
    int period_length = 0;
    /* изменение начального периода на противоположный - будет сброшен при первом чтении period_length */
    cur_period = !cur_period;

    int i, precipitation_type;
    precipitation_type = 0; // используется для выбора дождя или снега для пасмурного периода: 0 - дождь; 1 - снег
...
//    fprintf(stderr, "%d.%d.%d\n", weather_date.day, weather_date.month, weather_date.year);
    ...
    for ( i = 0; i < scen_info->turn_limit; i++ )
    {
        ...
        // определять дождь или снег только для пасмурного периода
        if ( cur_period == 1 )
