...
/*
    создать поверхность
    НЕ ДОЛЖЕН ИСПОЛЬЗОВАТЬСЯ, ЕСЛИ НЕ НАСТРОЕН SDLSCREEN
*/
...
/*
    blit поверхность с назначением DEST и источником SOURCE, использу€ его фактические настройки альфа и цвета
*/
void blit_surf(void)
{
 ...
 * (2, 3) --> S.......
 *
 * (1): верхний левый пиксель определяет пиксель прозрачности. Все пиксели
 ...
 * Цвет - либо # FF00FF для действительного глифа, либо # FF0000 для недопустимого.
 * глиф (в этом случае он не будет отображаться,
 * независимо от того, что он содержит в противном случае).
 *
 * (3): Для самого первого символа (2) имеет особое значение. Это обеспечивает
...
/*
====================================================================
Запишите строку в x, y и измените y так, чтобы она отображалась в
следующая строка.
====================================================================
*/
...
void init_sdl( int f )
{
    /* проверьте флаги: если ЗВУК не включен, флаг SDL_INIT_AUDIO не должен быть установлен */
#ifndef WITH_SOUND
    if ( f & SDL_INIT_AUDIO )
        f = f & ~SDL_INIT_AUDIO;
#endif
...
/** Получить список всех видеорежимов. Выделите @vmi и верните количество
 * записи. */
