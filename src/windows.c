...
Подсказка кнопок отображается в соответствующем ярлыке gui->.
...
/*
====================================================================
Проверьте событие движения и измените кнопки и метки.
...
/*
====================================================================
Установите буфер и отрегулируйте курсор x.
====================================================================
*/
void edit_show( Edit *edit, const char *newtext )
{
    ...
    /* очистить последний ключ */
    edit->last_sym = -1;
    /* очистить очередь событий SDL */
    event_clear_sdl_queue();
    /* отрегулировать cursor_x */
...
/*
====================================================================
Измените текстовый буфер в соответствии с юникодом и ключом.
====================================================================
*/
    ...
    switch ( keysym ) {
        ...
        default:
            ...
            if ( ( modifier & KMOD_LCTRL ) || ( modifier & KMOD_RCTRL ) ) {
                switch ( keysym ) {
                    case SDLK_u: /* CTRL+U: удалить строку */
                        edit->text[0] = '\0';
                        edit->cursor_pos = 0;
                        changed = 1;
                        break;
...
/*
====================================================================
Восстановите графику списка (lbox-> group-> frame).
====================================================================
*/
...
/** Выберите первую запись в списке и верните указатель или NULL, если он пуст. */
...
int lbox_handle_motion( LBox *lbox, int cx, int cy, void **item )
{
    ...
    if ( !lbox->group->frame->img->bkgnd->hide ) {
        if ( !group_handle_motion( lbox->group, cx, cy ) ) {
            /* проверьте, находится ли над ячейкой */
            if (lbox->items == NULL)
                return 0;
            for ( i = 0; i < lbox->cell_count; i++ )
                if ( FOCUS( cx, cy,
...
/*
====================================================================
handle_click внутренне обрабатывает щелчок вверх / вниз и возвращает элемент
если один был выбран.
Примечание. При нажатии на элемент @button не изменяется, но устанавливается @item.
Таким образом, если возвращается истина, необходимо проверить @item (@item == NULL,
была нажата кнопка и установлена ​​@button; @item не NULL, элемент был
clicked и @button может быть неопределенным).
====================================================================
*/
int lbox_handle_button( LBox *lbox, int button_id, int cx, int cy, Button **button, void **item )
{
    ...
    *item = 0;
    ...
    /* проверьте фокус, если не в списке, нечего делать */
    ...
    /* посмотреть, была ли нажата кнопка вверх / вниз */
    group_handle_button( lbox->group, button_id, cx, cy, button );

    /* ручка кнопки / колеса или выбор, если не скрыты */
    if ( !lbox->group->frame->img->bkgnd->hide ) {
...
/** Визуализировать элемент списка @item (который представляет собой простую строку) в ячейку списка
 * поверхность @buffer. */
    ...
    if ( !fdlg->lbox->group->frame->img->bkgnd->hide ) {
        ...
        if ( lbox_handle_button( fdlg->lbox, button_id, cx, cy, button, &item ) ) {
            if ( item ) {
                ...
                {
                    /* имя файла должно быть извлечено из структуры */
...
/*
====================================================================
Создайте диалог настройки сценария.
====================================================================
*/
                   ...
                   SDL_Surface *surf, int x, int y )
{
    ...
    /* группа с кнопкой управления человеком / процессором */
    ...
    /* группа с кнопкой выбора модуля ИИ */
    ...
    /* группа с кнопками настроек и подтверждения; id_conf - это идентификатор первой кнопки
     * на изображении conf_buttons */
...
/*
====================================================================
Создайте диалог настройки кампании.
====================================================================
*/
                   ...
                   SDL_Surface *surf, int x, int y )
{
    ...
    /* sи неиспользуемые поверхности как 0 */
    sdlg->list = 0;
    sdlg->ctrl = 0;
    sdlg->module = 0;

    /* gгруппа с кнопками настройки и подтверждения; id_conf - это идентификатор первой кнопки
     * на изображении conf_buttons */
    if ( ( sdlg->confirm = group_create( conf_frame, alpha, conf_buttons,
	...
	Button **pbtn )
{
	...
	if (lbox_handle_button(sdlg->select_lbox,bid,cx,cy,pbtn,&item))
		return 0; /* внутренние кнопки вверх / вниз */
	return 0;
}
