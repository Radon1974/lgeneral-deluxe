...
/*
====================================================================
Преобразование tile_id изображения PG в строку фрагмента карты
...
int maps_convert( int map_id )
{
	...
	/* при преобразовании кампании pg скопируйте названия карт в качестве резервных 
	 * не требуется: имена карт снова предоставляются в пакете 
    ...
    printf( "Maps...\n" );
    /* файл имени (попробуйте тот, который находится в lgc-pg как резервный) */
    ...
    for ( i = start; i <=end; i++ ) {
        ...
        if (( source_file = fopen_ic( path, "r" ) ) == NULL) {
            ...
            /* для пользовательской кампании могут присутствовать не все карты / сценарии, поэтому
             * не считайте это фатальным, просто продолжайте */
